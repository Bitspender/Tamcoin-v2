...
// Tamcoin: optional setting to unlock wallet for block minting only;
...
// Tamcoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Tamcoin: timestamp must not exceed spend time
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Tamcoin: not avatar mode
                        {
...
// Tamcoin: create coin stake transaction
...
// Tamcoin: check 'spent' consistency between wallet and txindex
// Tamcoin: fix wallet spent state according to txindex
...
// Tamcoin: disable transaction (only for coinstake)
